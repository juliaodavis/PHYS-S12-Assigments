...
<BR>
For today's lab, I decided to use the basic [touch sensor code](https://roberthart56.github.io/SCFAB/SC_lab/Sensors/CapacitiveTouch/CapacitiveTouch.txt), and add an averaging function to it so that I could measure the relative sensor value on each of the eight sections on the copper and then use a series of condiional statements to power the individual LED's. My touch sensor code with the averaging function implemented is below. 